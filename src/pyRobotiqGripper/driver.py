...
#Iport libraries
from pyRobotiqGripper import minimalmodbus as mm
import time
import serial
...
class RobotiqGripper( mm.Instrument ):
    ...
    def printInfo(self):
        """Display user-friendly English status of the gripper"""
        self.readAll()

        # Friendly English labels for parameters
        friendly_names = {
            "gOBJ": "Object Detection",
            "gSTA": "Gripper Status",
            "gGTO": "Action Status",
            "gACT": "Activation State",
            "kFLT": "Controller Fault",
            "gFLT": "Gripper Fault",
            "gPR":  "Requested Position",
            "gPO":  "Actual Position",
            "gCU":  "Motor Current"
        }

        print("\n\x1b[1;36m=== GRIPPER STATUS OVERVIEW ===\x1b[0m")  # Cyan title

        for key in self.paramDic:
            value = self.paramDic[key]
            label = friendly_names.get(key, key)
            desc = self.registerDic.get(key, {}).get(value, "")
            
            print(f"\x1b[1;33m{label:<22}\x1b[0m: {value}")
            if desc:
